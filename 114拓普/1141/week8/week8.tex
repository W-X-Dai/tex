...
\end{frame}
    ...
    \subsection{Introduction to Monte Carlo Method}
    \begin{frame}
...
int main(){
    ...
    \subsection{Implement of Monte Carlo method}
    \begin{frame}
        \frametitle{Estimation of Pi}
        Here, the task is:
        \begin{itemize}
            \item Generate N random points within a square of side length 1.
            \item Count how many points fall within the quarter circle of radius 1.
            \item Estimate the value of π using the ratio of points inside the quarter circle to the total number of points.
        \end{itemize}
    \end{frame}
    \begin{frame}[fragile]
        \frametitle{Generate N points within the Square}
        \begin{minted}{cpp}
random_device rd;
mt19937 g(rd()); 
...
for(int i = 0; i < N; i++){
    ...
    \end{frame}
    \begin{frame}[fragile]
        \frametitle{Count points within the Quarter Circle}
        \begin{minted}{cpp}
int inside_circle=0;
for(int i=0;i<N;i++){
    double x=rand_real(g);
    ...
    \end{frame}
    \begin{frame}[fragile]
        \frametitle{Estimate the value of Pi}
        The area of the quarter circle is $\pi(\cfrac{r}{2})^2= \cfrac{\pi}{4}$ (since $r=1$), and the area of the square is $1 \times 1 = 1$. Therefore, the ratio of the area of the quarter circle to the area of the square is $\cfrac{\pi}{4}$. Thus, we can estimate $\pi$ as follows:
        \begin{minted}{cpp}
...
cout<<"Estimated value of Pi: "<<pi_estimate<<"\n";
    ...
    \begin{frame}
        \frametitle{More}
        \begin{itemize}
            \item Try different values of N (e.g., 1000, 10000, 100000, 1000000) and observe how the estimate of π improves with larger sample sizes.
            \item Can you think of any improvements or optimizations to the code?
        \end{itemize}
    \end{frame}

    \section{Application: 1A2B}
    \subsection{Overview}
    \begin{frame}
    ...
    \begin{frame}[fragile]
        ...
        To generate n unique digits, generating them one by one is obvously not a good idea. Instead, we can use an array to store all digits from 0 to 9, shuffle them randomly, and then take the first n digits as the answer.
    \end{frame}
    \begin{frame}[fragile]
        \frametitle{Shuffle Function}
        \begin{minted}{cpp}
...
for(int i=0; i<n; i++) res+=num[i];
        \end{minted}
        With the above code, we can shuffle the whole string of the array \texttt{num}. After shuffling, we can take the first n elements as our answer.
        Remember to check if n is between 2 and 9.
    \end{frame}
    \begin{frame}[fragile]
        \frametitle{Check Input Validity}
        To ensure the player's guess is valid, we need to check three conditions:
        \begin{itemize}
            \item The input length must be equal to n.
            \item The input must only contain digits (1-9).
            \item All digits in the input must be unique.
        \end{itemize}
    \end{frame}
    \begin{frame}
        \frametitle{Calculate A and B}
        To calculate the number of A's and B's:
        \begin{itemize}
            \item For A's: Compare each digit of the player's guess with the corresponding digit of the answer. If they match, increment the A count.
            \item For B's: For each digit in the player's guess, check if it exists in the answer but in a different position. If it does, increment the B count.
        \end{itemize}
    \end{frame}
