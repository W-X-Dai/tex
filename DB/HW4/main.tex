...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{minted}{sql}
 Limit  (cost=66.83..196.66 rows=10 width=28) (actual time=0.199..0.492 rows=10 loops=1)
   ->  Result  (cost=66.83..560.17 rows=38 width=28) (actual time=0.198..0.491 rows=10 loops=1)
         ->  Sort  (cost=66.83..66.93 rows=38 width=20) (actual time=0.145..0.146 rows=10 loops=1)
               Sort Key: p1.depart_time
               Sort Method: top-N heapsort  Memory: 25kB
               ->  Hash Join  (cost=35.44..66.01 rows=38 width=20) (actual time=0.086..0.138 rows=62 loops=1)
                     Hash Cond: (p2.trip_id = p1.trip_id)
                     Join Filter: (p1.depart_time < p2.arrive_time)
                     Rows Removed by Join Filter: 60
                     ->  Seq Scan on pass p2  (cost=0.00..30.06 rows=195 width=12) (actual time=0.003..0.048 rows=195 loops=1)
                           Filter: (station_id = 1000)
                           Rows Removed by Filter: 1410
                     ->  Hash  (cost=34.08..34.08 rows=109 width=12) (actual time=0.071..0.072 rows=122 loops=1)
                           Buckets: 1024  Batches: 1  Memory Usage: 14kB
                           ->  Seq Scan on pass p1  (cost=0.00..34.08 rows=109 width=12) (actual time=0.010..0.062 rows=122 loops=1)
