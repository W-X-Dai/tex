...
commit;
    ...
    \begin{enumerate}
		...
		\item Following is one possible conflict-serializable schedule that would lead to deadlock under 2PL, a deadlock occurs because T1 holds a shared lock on X while T2 attempts to upgrade its shared lock on X to an exclusive lock. At the same time, T2 holds a shared lock on Z, and T1 attempts to acquire an exclusive lock on Z. Both transactions enter a circular wait condition, and since neither can release its lock during the 2PL shrinking phase, a deadlock is formed.
