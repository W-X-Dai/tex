...
create index idx_tda on reserved_ticket(trip_id, depart_station_id, arrive_station_id);
    ...
    \begin{enumerate}
        ...
        \end{figure}
    \end{enumerate}
    \item I noticed that the after creating the multicolumn index, the estimated cost is significantly reduced compared to the single-column indexes created earlier. This indicates that the multicolumn index is more effective in optimizing the query performance for this specific SQL statement. Therefore, I decided to try to assemble another columns used in the sub query to see if it can further improve the performance. In the multicolumn index of PostgreSQL, the order of the columns matters. Since the subquery filters on \texttt{trip\_id}, \texttt{arrive\_station\_id}, \texttt{depart\_station\_id}, and \texttt{travel\_date}, I decided to create an index with these columns in the order that best matches the filtering conditions.
    
    Following may one possible index method to optimize the query performance for the given SQL statement:
    \begin{minted}{sql}
