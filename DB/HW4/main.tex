...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Transaction A: Update balance to 1000 of account\_id 1
        \begin{minted}{sql}
begin;
...
commit;
    ...
    Following are the screenshots of each step. Left panel shows Transaction A, right panel shows Transaction B. Figure \ref{fig:1-1} shows the original status of the accounts table, we can see the balance of account\_id 1 is 999. Figure \ref{fig:1-2} shows Transaction A updates balance to 1000 of account\_id 1 but does not commit yet, so that Transaction B still reads the old balance (999). Figure \ref{fig:1-3} shows Transaction A commits, and then Transaction B reads the new balance (1000) of account\_id 1.
    \begin{figure}[H]
        \centering
    ...
    \begin{enumerate}
		...
		\item As shown in the table below, when T2 attempts to upgrade its lock on X to a write lock, it is blocked by T1's existing read lock on X. T1 will be blocked when it later attempts to upgrade its lock on X because T2 holds the read lock on X. This creates a circular wait condition, leading to a deadlock between T1 and T2.
        ...
        \begin{tabular}{l l l}
        \toprule
        \textbf{T1} & \textbf{T2} & \textbf{Phase} \\ \midrule
        \texttt{read\_lock(X)} & & Expanding \\
        \texttt{read\_item(X)}  &                          &  \\ 
                                        &  \texttt{read\_lock(X)}  & Expanding \\
                                        &  \texttt{read\_item(X)}  &  \\
                                        &  \texttt{write\_item(X)} & blocked, T1 holds S(X) \\

        \texttt{read\_lock(Y)} &                          & Expanding \\
        \texttt{read\_item(Y)}  &                          &  \\
        \texttt{read\_lock(Z)}  &                          & Expanding \\
        \texttt{read\_item(Z)}  &                          &  \\
        \texttt{write\_item(Z)}  &                          & Expanding \\
        \texttt{write\_item(Z)}  &                          &  \\
        \texttt{write\_lock(X)} &                          & blocked, T2 holds S(X) \\
    ...
    \item 
    \begin{enumerate}
        \item This SQL statement queries the top 10 reserved sales for trips that depart from station\_id 1030 and arrive at station\_id 1000 during the period from 2023-08-01 to 2023-08-31 order by departure time, and the train must depart after 06:00. It joins the PASS table with itself on trip\_id, where p1 represents the departure station (1030) and p2 represents the arrival station (1000). After joining, it selects each trip's departure and arrival time, and uses a subquery to count how many tickets were reserved for that same trip and same station pair within the specified date range. Finally, the results are ordered by departure time and limited to the first 10 row
...
create index idx_tda on reserved_ticket(trip_id, depart_station_id, arrive_station_id);
        ...
        \item As my feeling, the index with higher estimated cost would lead to a slower query performance. To quantify the actual performance difference, I run \texttt{explain analyze} on the SQL statement for each index created above. For each index, I execute the query 5 times and take the mean of the execution time (in milliseconds). The results are shown in the table below:
        \begin{table}[H]
            \centering
            \begin{tabular}{p{5.5cm} r r}
            \toprule
            \textbf{Index} & \textbf{Mean (ms)} & \textbf{Estimated Cost} \\
            \midrule
            No index & 1097.573 & 3,750,384.97 \\
            trip\_id & 219.085 & 1,024,884.52 \\
            depart\_station\_id & 944.488 & 1,559,993.73 \\
            arrive\_station\_id & 1035.202 & 1,731,233.13 \\
            trip\_id, depart\_station\_id, arrive\_station\_id & 4.151 & 22,244.14 \\
            \bottomrule
            \end{tabular}
        \end{table}   

        With the experiments above, we can observe that the index with lower estimated cost generally leads to better actual performance.
    \end{enumerate}
    \item 
    \begin{enumerate}
