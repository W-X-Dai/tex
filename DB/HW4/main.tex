...
\begin{enumerate}
    ...
    \begin{enumerate}
		...
		\label{tab:2pl_deadlock}
		\begin{tabular}{l l l}
		\toprule
		\textbf{Step} & \textbf{Operation} & \textbf{Note / Lock Status} \\
		\midrule
		1  & $T_1$: \texttt{read\_lock(X)}   & $T_1$ holds $X$-R \\
		2  & $T_1$: \texttt{read\_item(X)}   & - \\
		3  & $T_2$: \texttt{read\_lock(X)}   & $T_1, T_2$ share $X$-R \\
		4  & $T_2$: \texttt{read\_item(X)}   & - \\
		5  & $T_2$: \texttt{write\_lock(X)}  & \textbf{blocked}, $T_1$ holds $X$-R \\
		6  & $T_1$: \texttt{read\_lock(Y)}   & $T_1$ holds $Y$-R \\
		7  & $T_1$: \texttt{read\_item(Y)}   & - \\
		8  & $T_1$: \texttt{read\_lock(Z)}   & $T_1$ holds $Z$-R \\
		9  & $T_1$: \texttt{read\_item(Z)}   & - \\
		10 & $T_1$: \texttt{write\_lock(Z)}  & \textbf{blocked}, $T_2$ holds $Z$-R \\
		\bottomrule
		\end{tabular}
		\end{table}
...
create index idx_tda on reserved_ticket(trip_id, depart_station_id, arrive_station_id);
    ...
    \begin{enumerate}
        ...
        \end{figure}
    \end{enumerate}
    \item Following may one possible index method to optimize the query performance for the given SQL statement:
    \begin{minted}{sql}
create index idx_reserve_optimized on reserved_ticket(depart_station_id, arrive_station_id, travel_date, trip_id);
	\end{minted}
